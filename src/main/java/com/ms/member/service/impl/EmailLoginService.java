...
@Service
@AllArgsConstructor
public class EmailLoginService implements MemberLoginUseCase {
  ...
  public LoginToken login(MemberLoginCommand command) throws MemberNotFoundException {
    var member = memberRepository.findByEmailAndPassword(command.getKey(), command.getPassword())
